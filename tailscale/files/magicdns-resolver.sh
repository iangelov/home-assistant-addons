...
prepare_egress() {
  ...
  options+=("--listen-address=${EGRESS_ADDRESS}" '--address=/#/' "--server=/controlplane.tailscale.com/${ha_dns}" "--server=/log.tailscale.com/${ha_dns}" "--server=/acme-v02.api.letsencrypt.org/${ha_dns}")
  start_dnsmasq "${EGRESS_PID}" "${options[@]}"
...
start_ingress() {
  ...
  start_dnsmasq "${INGRESS_PID}" "${options[@]}"
...
case "${1:-}" in
  ...
  cleanup) cleanup ;;
  *)
    printf 'Usage: %s {prepare-egress|start-ingress|cleanup}\n' "${0##*/}" >&2
    exit 2
    ;;
esac
